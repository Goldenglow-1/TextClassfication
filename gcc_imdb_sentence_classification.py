...
class GCNN(nn.Module):
    def __init__(self, vocab_size=VOCAB_SIZE, embedding_dim=64, num_classes=2):
        ...
        self.output_linear_1 = nn.Linear(64, 128)
        self.output_linear_2 = nn.Linear(128, num_classes)

    def forward(self, word_index):
        #被定义GCN网络的算子操作流程，基于句子单词ID输入得到分类logits输出
        ...
        #3.池化并经过全连接层
        pool_output = torch.mean(H, dim=-1)#平均池化， 得到[bs, 128]
        linear_output = self.output_linear1(pool_output)
        logits = self.output_linear2(linear_output)#]bs, num_classes]
...
class TextClassificationModel(nn.Module):
    ...
    def forward(self, token_index):
        ...
        return self.fc(embedded)#shape:[bs, num_class]
...
'''这个 collate_fn 函数用于处理一个批次的样本，将评论文本转换为标记索引并进行填充，同
时将标签编码为相应的标签。这是一个常见的数据预处理步骤，用于准备数据输入到深度学习模
型中。'''
def collate_fn(batch):
    ...
    return (torch.tensor(target).to(torch.int64), torch.tensor(token_index).to(torch.int43))


def train(train_data_loader, eval_data_lodaer, model, optimizer, num_epoch, log_step_interval, save_step_interval, eval_step_interval, 
          save_path, resume=''):
    '''此处data_loader是map_style dataset'''
    start_epoch = 0
    start_step = 0
    if resume != '':
        #加载之前训练过的模型的参数文件
    ...
    for epoch_index in range(start_epoch, num_epoch):
        ...
        for batch_index, (target, token_index) in enumerate(train_data_loader):
            ...
            if step % eval_step_interval == 0:
                ...
                total_acc_account = 0
                total_account = 0
                for eval_batch_index, (eval_target, eval_token_index) in enumerate(eval_data_lodaer):
...
if __name__ == "__main__":
    ...
    train(train_data_loader, eval_data_loader, model, optimizer, num_epoch=10, log_step_interval=20, save_step_interval=500, 
